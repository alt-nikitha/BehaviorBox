...
# else
#   if [ -n "$revision" ]; then
#     sbatch scripts/data_generation/vllm_host.sh --model_id="$model_id" --model_name="$model_name" --revision="$revision"
#   else
#     sbatch scripts/data_generation/vllm_host.sh --model_id="$model_id" --model_name="$model_name"
#   fi
# fi
...
#   else
#     echo "Waiting for scripts/data_generation/tmp/${model_name}/host_port.txt to be created..."
#     sleep 5
#   fi
# done
...
if [ -f "scripts/data_generation/tmp/${model_name}/host_port.txt" ]; then
  ...
  if curl -f -s http://${model_addr}/health > /dev/null 2>&1; then
    echo "Found existing vLLM server for ${model_name} at ${model_addr}"
  else
    echo "Found host_port.txt but server not responding, will launch new server"
    model_addr=""
  fi
fi

# Launch vLLM server only if not already running
if [ -z "$model_addr" ]; then
  ...
  echo "Waiting for the vLLM server to start..."
  # Poll for address
  # Address is written to scripts/data_generation/tmp/${model_name}/host_port.txt
  model_addr=""
  while [ -z "$model_addr" ]; do
    if [ -f "scripts/data_generation/tmp/${model_name}/host_port.txt" ]; then
      model_addr=$(cat "scripts/data_generation/tmp/${model_name}/host_port.txt")
  ...
  echo "Using existing vLLM server at ${model_addr}"
fi
