...
folder = "/home/nsrikant/bbox_outputs/sae_outputs_blimp/n_moreearly/n_moreearly_seed=42_ofw=_N=3000_k=50_lp=None"
...
def max_jump(values):
    ...
    if len(vals) < 2:
        return (0, -1)
    diffs = np.diff(vals)
    jump_idx = np.argmax(diffs)  # index of max increase
...
# -----------------------------
app = Dash(__name__)
app.title = "Feature Trend + Activations - BLIMP"

app.layout = html.Div([
    html.H2("Feature Trends Across Models - BLIMP", style={"textAlign": "center"}),
    ...
    dcc.Slider(
        id="error-slider",
        min=0.0,
        max=1.0,      # adjust based on your probability scale
        step=0.01,
        value=0.25,     # default = no filtering
...
@app.callback(
    ...
    Input("metric-dropdown", "value"),
    Input("error-slider", "value")   # <-- new input
)
def update_features(selected_trend, metric_choice, max_allowed_error):
    trend_col = f"Trend ({metric_choice})"
    ...
    for _, row in sub.iterrows():
        ...
        fig = px.line(plot_df, x="Model", y=y_label,  markers=True, title=f"Feature {row['Feature']} ({y_label}); Range: {probs_range}")
